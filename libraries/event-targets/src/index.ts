...
export type EventName<EventMap extends EventMapBase> = Extract<
  keyof EventMap,
  string
>
...
> extends EventTarget {
  readonly #listeners = new Map<
    string,
    Map<EventHandler<unknown>, ListenerRecord<unknown>>
  >()
  ...
  off<K extends EventName<EventMap>>(
    ...
    const record = eventListeners.get(
  ...
  async emit<K extends EventName<EventMap>>(
    ...
    if (options.throwOnError && errors.length > 0) {
      throw new AggregateError(
        errors,
        `One or more handlers failed for event "${eventName}".`,
      )
    }
   ...
   */
  emitSync<K extends EventName<EventMap>>(
    event: K,
    data: EventMap[K],
  ): RuntimeEvent<EventMap[K]> {
   ...
   */
  listeners<K extends EventName<EventMap>>(
    event: K,
  ): Array<EventHandler<EventMap[K]>> {
    ...
    return [...records.values()].map(
      (record) => record.handler as EventHandler<EventMap[K]>,
    )
  }
  ...
  setWarningHandler(
    handler: EventEmitterOptions['onWarning'],
  ): this {
  ...
  setErrorHandler(
    handler: EventEmitterOptions['onError'],
  ): this {
...
export class EventBus {
  ...
  static getInstance(): EventEmitter<Record<string, unknown>> {
    this.#instance ??= createEventEmitter()
  ...
  static setInstance(
    instance: EventEmitter<Record<string, unknown>>,
  ): void {
    this.#instance?.removeAllListeners()
