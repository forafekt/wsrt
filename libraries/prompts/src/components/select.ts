...
import { SelectController } from "../core/mod.ts";
import {
	type CommonOptions,
	S_BAR,
	S_BAR_END,
...
export const select = <Value>(opts: SelectOptions<Value>) => {
	const opt = (
		...
		switch (state) {
			case "disabled":
				return `${colors.gray(S_RADIO_INACTIVE)} ${computeLabel(label, colors.gray)}${
			...
			case "cancelled":
				return `${computeLabel(label, (str) => colors.strikethrough(colors.dim(str)))}`;
			default:
				return `${colors.dim(S_RADIO_INACTIVE)} ${computeLabel(label, colors.dim)}`;
		}
	};
