import { wrapAnsi } from "@wsrt/ansi-tools";
...
export const box = (message = "", title = "", opts?: BoxOptions) => {
	...
	output.writeSync(
		encoder.encode(
			`${linePrefix}${symbols[0]}${hSymbol.repeat(
				titlePaddingLeft,
	...
	for (const line of wrappedLines) {
		...
		output.writeSync(
			encoder.encode(
				`${linePrefix}${vSymbol}${" ".repeat(leftLinePadding)}${line}${" ".repeat(
					rightLinePadding,
				)}${vSymbol}`,
			),
		);
	...
	output.writeSync(
		encoder.encode(`${linePrefix}${symbols[2]}${hSymbol.repeat(innerWidth)}${symbols[3]}\n`),
	);
};
