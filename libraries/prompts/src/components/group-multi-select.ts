...
import { GroupMultiSelectController } from '../core/mod.ts'
import {
  type CommonOptions,
  S_BAR,
  S_BAR_END,
...
export const groupMultiselect = <Value>(opts: GroupMultiSelectOptions<Value>) => {
  ...
  const opt = (
    ...
    if (state === 'active') {
      return `${spacingPrefix}${colors.dim(prefix)}${colors.cyan(
        S_CHECKBOX_ACTIVE,
      )} ${label}${option.hint ? ` ${colors.dim(`(${option.hint})`)}` : ''}`
    }
    if (state === 'group-active') {
      return `${spacingPrefix}${prefix}${colors.cyan(S_CHECKBOX_ACTIVE)} ${colors.dim(label)}`
    }
    if (state === 'group-active-selected') {
      return `${spacingPrefix}${prefix}${colors.green(S_CHECKBOX_SELECTED)} ${colors.dim(label)}`
    }
    if (state === 'selected') {
      const selectedCheckbox = isItem || selectableGroups ? colors.green(S_CHECKBOX_SELECTED) : ''
      return `${spacingPrefix}${colors.dim(prefix)}${selectedCheckbox} ${colors.dim(
        label,
    ...
    if (state === 'active-selected') {
      return `${spacingPrefix}${colors.dim(prefix)}${colors.green(
        S_CHECKBOX_SELECTED,
      )} ${label}${option.hint ? ` ${colors.dim(`(${option.hint})`)}` : ''}`
    }
    if (state === 'submitted') {
    ...
    const unselectedCheckbox = isItem || selectableGroups ? colors.dim(S_CHECKBOX_INACTIVE) : ''
    return `${spacingPrefix}${colors.dim(prefix)}${unselectedCheckbox} ${colors.dim(label)}`
  }
  const required = opts.required ?? true

  return new GroupMultiSelectController({
    ...
    render() {
      ...
      switch (this.state) {
        case 'submit': {
          ...
          const optionsText =
            selectedOptions.length === 0 ? '' : `  ${selectedOptions.join(colors.dim(', '))}`
        ...
        case 'error': {
          const footer = this.error
            ...
            .join('\n')
          return `${title}${colors.yellow(S_BAR)}  ${this.options
