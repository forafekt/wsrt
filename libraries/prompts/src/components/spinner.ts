import { colors, cursor, erase, wrapAnsi } from '@wsrt/ansi-tools'
...
export const spinner = ({
  ...
  const start = (msg = ''): void => {
    ...
    loop = setInterval(() => {
      ...
      const frame = styleFn?.(frames[frameIndex]) ?? frames[frameIndex]
  ...
  const _stop = (msg = '', code = 0, silent: boolean = false): void => {
    ...
    if (!silent) {
      if (indicator === 'timer') {
        output.writeSync(encoder.encode(`${step}  ${_message} ${formatTimer(_origin)}\n`))
      } else {
