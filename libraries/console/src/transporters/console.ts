...
function renderTree(value: unknown, options: RenderTreeOptions = {}): string {
	const { rootLabel, indent = "  ", maxDepth = 10 } = options;
...
function appendTreeValue({
	...
	if (depth >= maxDepth) {
		lines.push(`${prefix}${ansi.colors.gray("└─")} ${ansi.colors.gray("[Max depth reached]")}`);
		return;
	}

	if (typeof value === "object" && value !== null) {
		if (seen.has(value)) {
			lines.push(`${prefix}${ansi.colors.gray("└─")} ${ansi.colors.gray("[Circular]")}`);
			return;
		}
	...
	if (entries.length === 0) {
		lines.push(`${prefix}${ansi.colors.gray("└─")} ${formatEmptyCollection(value)}`);
		return;
	}
	...
	entries.forEach(([key, childValue], index) => {
		...
		if (typeof childValue === "object" && childValue !== null) {
			if (seen.has(childValue)) {
				lines.push(`${nextPrefix}${ansi.colors.gray("└─")} ${ansi.colors.gray("[Circular]")}`);
				return;
			}
		...
		if (childEntries.length === 0) {
			lines.push(`${nextPrefix}${ansi.colors.gray("└─")} ${formatEmptyCollection(childValue)}`);
			return;
		}
...
function appendEntries({
	...
	entries.forEach(([key, value], index) => {
		...
		if (!childEntries) {
			lines.push(
				`${prefix}${ansi.colors.gray(branch)} ` + `${formatKey(key)}: ${formatPrimitive(value)}`,
			);
			return;
		}
		...
		if (childEntries.length === 0) {
			lines.push(`${nextPrefix}${ansi.colors.gray("└─")} ${formatEmptyCollection(value)}`);
			return;
		}
...
function getEntries(value: unknown): Array<[string, unknown]> | null {
	...
	if (value instanceof Map) {
		return Array.from(value.entries(), ([key, childValue]) => [String(key), childValue]);
	}

	if (value instanceof Set) {
		return Array.from(value.values(), (childValue, index) => [`[${index}]`, childValue]);
	}
...
function insert(
	...
	return inputs.map(({ text, color }) => (color ? color(text) : text)).join(" ");
}
