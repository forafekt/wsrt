...
export class ConsoleUiTransport implements ConsoleLogTransport {
    ...
    if (
      ...
      lines.push(
        renderStructuredValue(entry.context, {
          ...
          compactEmptyCollections:
            this.options.compactEmptyCollections,
  ...
  private renderHeader(entry: ConsoleLogEntry): string {
    ...
    if (this.options.showTimestamp) {
      parts.push(
        ansi.colors.gray(formatTimestamp(entry.timestamp)),
      );
    }
  ...
  private getWidth(): number {
    ...
    return Math.max(
      60,
      Math.min(terminalWidth, this.options.maxWidth),
    );
  }
}
...
function renderNamedSection(
  ...
  if (isEmptyCollection(value)) {
    return `${ansi.colors.bold(title.padEnd(13))} ${ansi.colors.gray(
      emptyCollectionLabel(value),
    )}`;
  }
...
function renderArraySection(
  ...
  if (
    values.length >= options.tableMinRows &&
    ...
    if (columns.length > 0) {
      return [
        renderSectionHeading(title, values.length),
        indentBlock(
          renderTable(records, columns, options.maxWidth - 2),
          2,
        ),
      ].join("\n");
    }
...
function renderKeyValueSection(
  ...
  const keyWidth = Math.max(
    ...entries.map(([key]) => humanizeKey(key).length),
  );
  ...
  for (const [key, value] of entries) {
    lines.push(
      `  ${ansi.colors.gray(
        humanizeKey(key).padEnd(keyWidth),
      )}  ${formatScalar(value)}`,
    );
  }
...
function renderTable(
  ...
  const rows = records.map((record) =>
    columns.map((column) => formatTableValue(record[column])),
  );
  ...
  const header = columns
    .map((column, index) =>
      ansi.colors.gray(
        truncate(column.toUpperCase(), widths[index]).padEnd(
          widths[index],
        ),
      ),
    )
  ...
  const body = rows.map((row) =>
    row
      .map((cell, index) =>
        padAnsi(truncateAnsi(cell, widths[index]), widths[index]),
      )
      .join("  "),
  );
...
function renderError(
  ...
  const lines = [
    `${ansi.colors.bold(error.name)}: ${ansi.colors.red(
      error.message,
    )}`,
  ];
  ...
  if (error.cause !== undefined) {
    ...
    lines.push(
      indentBlock(renderError(error.cause, options), 2),
    );
  }
  ...
  if (customEntries.length > 0) {
    lines.push("");
    lines.push(
      renderKeyValueSection("Details", customEntries),
    );
  }
...
function formatScalar(value: unknown): string {
  ...
  if (typeof value === "boolean") {
    return value
      ? ansi.colors.green("true")
      : ansi.colors.gray("false");
  }
...
function formatTimestamp(
  timestamp: ConsoleLogEntry["timestamp"],
): string {
...
function shrinkColumnsToWidth(
  ...
  while (
    ...
    for (let index = 0; index < widths.length; index += 1) {
      if (
        widths[index] > widths[largestIndex] &&
        widths[index] > 8
      ) {
        largestIndex = index;
      }
...
function padAnsi(value: string, width: number): string {
  return value + " ".repeat(
    Math.max(0, width - visibleLength(value)),
  );
}
