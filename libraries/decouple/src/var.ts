...
export class DecoupledVar {
	...
	private resolve<T>(caster: CastFn<T>, defaultValue?: T) {
		...
		try {
			...
			throw new DecoupleError(`Error parsing "${this.key}": ${(err as Error).message}`);
		}
	}
	...
	enum<const T extends readonly string[]>(values: T, defaultValue?: T[number]): T[number] {
	...
	match(pattern: RegExp, defaultValue?: string) {
		return this.resolve((v) => {
			if (!pattern.test(v)) {
	...
	required() {
		if (typeof this.value !== "string") {
			throw new DecoupleError(`Missing required config value: ${this.key}`);
		}
		return this.value;
	}
