...
const violations = files.flatMap((file) => banned.filter((name) => fs.readFileSync(file, 'utf8').includes(name)).map((name) => `${file}: ${name}`))
if (violations.length) throw new Error(`Obsolete imports:\n${violations.join('\n')}`)
console.log(`Architecture valid: ${packages.size} packages, ${edges.length} workspace edges, no cycles or obsolete imports`)
function walk(root) {
