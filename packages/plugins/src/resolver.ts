		...
		const plugin =
			typeof reference === "string"
				...
				: "id" in reference
					...
					: reference.provider;
		try {
			plugins.push(...(await resolveWorkspacePlugins([reference], workspaceRoot)));
		} catch (cause) {
			diagnostics.push({
				code:
					cause instanceof Error && cause.message.startsWith("WSRT_PLUGIN_PACKAGE_NOT_FOUND")
						? "plugin.not_found"
						: "plugin.load_failed",
				severity: "error",
				plugin,
...
export async function resolveWorkspacePlugins(
	...
	for (const reference of references) {
		...
		const packageName = typeof reference === "string" ? reference : reference.provider;
		const options = typeof reference === "string" ? undefined : reference.options;
		let url: string;
		try {
			url = packageName.startsWith("file:")
				...
				: path.isAbsolute(packageName)
					...
					: packageName.startsWith(".")
						? pathToFileURL(path.resolve(path.dirname(fileURLToPath(parent)), packageName)).href
		...
		try {
			...
			const plugin = typeof factory === "function" ? await factory(options) : factory;
