...
export type RuntimeEventBus = {
  on: <Name extends RuntimeEventName>(
    name: Name,
    listener: (event: RuntimeEventMap[Name]) => void,
  ) => () => void
...
export type RuntimeTimeline = {
  record: <Name extends RuntimeEventName>(
    name: Name,
...
export type RuntimeService = Omit<
  RuntimeServiceDefinition,
  'start' | 'stop' | 'health' | 'logs' | 'metrics' | 'environment'
> & {
  name: string
...
export type RuntimeQuery = {
  ...
  packages: (query?: { search?: string; limit?: number }) => WorkspacePackage[]
  services: () => RuntimeService[]
  graph: (query?: RuntimeGraphQuery) => { nodes: RuntimeGraphNode[]; edges: RuntimeGraphEdge[] }
  diagnostics: (query?: { level?: DiagnosticLevel; limit?: number }) => WsrtDiagnostic[]
  artifacts: () => RuntimeArtifact[]
  events: (query?: { name?: RuntimeEventName; limit?: number }) => RuntimeTimelineEntry[]
  timeline: (limit?: number) => RuntimeTimelineEntry[]
  config: () => { root: string; configFile?: string; sources: ConfigSource[]; config: WsrtConfig }
...
export type RuntimeTaskRegistry = {
  ...
  run: (id: string, context?: { args?: string[]; input?: unknown }) => Promise<unknown>
}
...
export type WsrtPlugin = {
  ...
  beforeDev?: (context: WsrtPluginContext & { project?: RuntimeProject }) => void | Promise<void>
  afterDev?: (
    context: WsrtPluginContext & { project?: RuntimeProject; handle?: ProjectHandle },
  ) => void | Promise<void>
  adapterCreated?: (
    context: WsrtPluginContext & { adapter: ProjectAdapter },
  ) => void | Promise<void>
  projectStarted?: (
    context: WsrtPluginContext & { project: RuntimeProject; handle: ProjectHandle },
  ) => void | Promise<void>
...
export type WsrtModuleReference =
  | string
  | {
      path?: string
      package?: string
...
export type ProjectProcessConfig = Omit<ProjectConfig, 'processes'> & { name?: string }
...
export type WsrtConfig = {
  ...
  runtime?: { environment?: RuntimeEnvironment; profile?: string }
  ...
  report?: { file?: string; pretty?: boolean }
}
...
export type WorkspaceGraph = {
  nodes: Array<{ id: string; root: string; kind?: string; metadata?: Record<string, unknown> }>
  edges: Array<{ from: string; to: string; type: 'workspace' | 'external' | (string & {}); metadata?: Record<string, unknown> }>
}
...
export type RuntimeGraphModel = WorkspaceGraph & {
  ...
  query: (query?: RuntimeGraphQuery) => { nodes: RuntimeGraphNode[]; edges: RuntimeGraphEdge[] }
...
export type WorkspaceRuntimeState = {
  ...
  services: RuntimeService[]
  tsconfig: { enabled: boolean; mode: SyncMode; files: SyncFileStatus[] }
  manifests: { enabled: boolean; mode: SyncMode; files: SyncFileStatus[] }
